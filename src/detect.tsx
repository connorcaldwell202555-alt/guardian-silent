...
const App = () => {
  ...
  const vibrate = (pattern: number[]) => {
    if (typeof navigator !== 'undefined' && 'vibrate' in navigator) {
      // @ts-ignore navigator.vibrate exists in browsers
  ...
  useEffect(() => {
    if (typeof navigator !== 'undefined' && 'getBattery' in navigator) {
      // @ts-ignore getBattery exists in some browsers
  ...
  useEffect(() => {
    ...
    const handleMotion = (e: any) => {
      ...
      if (currTime - lastUpdate > 100) {
        ...
        // Calculate Force
        const force = Math.abs((acc.x ?? 0) + (acc.y ?? 0) + (acc.z ?? 0) - (lastX ?? 0) - (lastY ?? 0) - (lastZ ?? 0)) / diffTime * 10000
  ...
  const openSilentText = (recipient = '911', customBody: string | null = null) => {
  ...
  if (isFakeDead) {
    return <div className="fixed inset-0 bg-black z-[200] cursor-none" onDoubleClick={() => { setPinReason('revivePhone'); setShowPinPad(true) }} />
  }
  ...
  return (
    <div className={getMainContainerClass()}>
      ...
      <div className="flex-grow overflow-y-auto hide-scrollbar relative z-10">
          ...
          <div className="h-full flex flex-col items-center justify-center space-y-6 py-4">
            {isSOSActive ? (
              <div className="w-full space-y-4 text-center animate-in zoom-in duration-300">
                ...
                <button onClick={() => { setPinReason('cancelSOS'); setShowPinPad(true) }} className="w-full py-4 bg-red-950/50 border border-red-900/50 rounded-2xl text-red-400 font-bold text-xs tracking-widest active:bg-red-900">STOP EMERGENCY (PIN)</button>
              </div>
            ) : (
              <>
        ...
        {view === 'settings' && <SettingsView onBack={() => setView('main')} shakeToSos={shakeToSos} setShakeToSos={setShakeToSos} voiceTriggerEnabled={voiceTriggerEnabled} setVoiceTriggerEnabled={setVoiceTriggerEnabled} silentRecording={silentRecording} setSilentRecording={setSilentRecording} fallDetection={fallDetection} setFallDetection={setFallDetection} token={trackingToken} />}
...
const SilentTextView: React.FC<{ onBack: () => void; onSend: (r: string) => void; contacts: { id: number; name: string; phone: string }[]; token: string }> = ({ onBack, onSend, contacts, token }) => (
...
const SettingsView: React.FC<{
  onBack: () => void
  shakeToSos: boolean
  setShakeToSos: (v: boolean) => void
  voiceTriggerEnabled: boolean
  setVoiceTriggerEnabled: (v: boolean) => void
  silentRecording: boolean
  setSilentRecording: (v: boolean) => void
